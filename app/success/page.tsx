...
function SuccessContent() {
  ...
  return (
    <>
      ...
      <main className="flex-grow flex flex-col items-center justify-center px-container-margin py-section-gap max-w-2xl mx-auto w-full">
        <div className="relative mb-8">
          <div className="absolute -inset-12 opacity-30 blur-3xl bg-primary/20 rounded-full animate-pulse" />
          <div className="relative flex items-center justify-center w-40 h-40 md:w-56 md:h-56 bg-white rounded-full border-4 border-[#6B21A8] shadow-[8px_8px_0px_0px_#000000] z-20">
            <span
              className="material-symbols-outlined text-[80px] md:text-[120px] text-[#22c55e]"
              style={{ fontVariationSettings: "'FILL' 1" }}
            >
              check_circle
            </span>
            <div className="absolute -top-4 -right-4 w-12 h-12 bg-[#FFD700] rounded-full flex items-center justify-center border-2 border-black">
        ...
        <div className="w-full bg-[#F9FAFB] border border-[#E9D5FF] rounded-xl p-card-padding space-y-6">
          <h2 className="font-label-bold text-label-bold text-primary uppercase tracking-widest">
            Transaction Details
          </h2>
          <div className="flex flex-wrap gap-3">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-outline-variant rounded-full">
              <span className="material-symbols-outlined text-primary text-sm">person</span>
              <span className="font-label-md text-label-md text-on-surface">Voted as: {name}</span>
            </div>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-outline-variant rounded-full">
              <span className="material-symbols-outlined text-primary text-sm">group</span>
              <span className="font-label-md text-label-md text-on-surface">Unit: {unit}</span>
            </div>
          </div>
